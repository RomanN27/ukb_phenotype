...
class SubstanceAbuseNonAlcoholic(PhenoType):

    def __init__(self):
        super().__init__(
            name="SubstanceAbuseNonAlcoholic",
            icd9_codes=["3040"],
            icd10_codes=[
            ...
            sr_codes=["1409", "1410"],
            associated_field_numbers=[20453, 20454, 20002, 20456, 20503]
        )
    ...
    def query(self, df: DataFrame) -> DataFrame:
        ...
        substance_addiction = illness_dependency | illicit_drug_addiction | medication_addiction

        df = df.withColumn(self.name, substance_addiction)

        return df


class AlcoholAbuse(PhenoType):

    def __init__(self):
        super().__init__(
            name="AlcoholAbuse",
            icd9_codes=["3039"],
            icd10_codes=["F10.1", "F10.2", "F10.3", "F10.4"],
            sr_codes=["1408"],
            associated_field_numbers=[20414, 20403, 20416, 20413, 20407, 20412, 20409, 20408, 20411, 20405, 20406]
        )
    ...
    def query(self, df: DataFrame) -> DataFrame:
        ...
        df = df.withColumn(self.name, alcohol_addiction | (col("AUDIT_score") >= 15))
        return df
